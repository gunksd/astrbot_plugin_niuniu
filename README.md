...
- 3.3修复 遍历词典时数据错误的问题（可能还会有其他地方出现相同问题 有bug提交issues（**建议**）或加群@长安某）修复yml中部分文本无法被正确触发的问题，调整胜率判断 现在除特殊事件外 胜率为      **50%（基础） +  （-30% ~ 30%）[长度影响]+ （-20% ~ 20%）[硬度影响]** 最终胜率在 20% -80% 之间 修复比划时出现占位符的问题（可能会有其他地方出错）
- 3.4修复 `我的牛牛` yml文件与py中不一致导致的报错
## 四、更新计划
